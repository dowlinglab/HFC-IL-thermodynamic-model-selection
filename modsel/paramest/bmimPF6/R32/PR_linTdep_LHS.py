...
# #Try initialized from PR noTdep using initial scaled parameters for Params 1-2, LHS for params 3-4 
# lhs = pd.read_csv('Data/Fits/LHS_Fits/PR_linTdep_127_scLHS.csv')

lhs = pd.read_csv('../../LHS_250x8.txt')
lhs = lhs[['Param1','Param2','Param3','Param4']]
...
for i in range(len(lhs)):
    ...
    try:
        ...
        eps = 0.1, scaling_fac = 1e-9, filename='Data/Fits/LHS_Fits/Ipopt_Output/PR_linTdep/PR_linTdep_LHS_ipopt_params_'+str(i)+'.txt')

        print(parameters)

        result[i,:] = parameters[0], parameters[1], parameters[2], parameters[3], obj_value
    except:
        parameters, obj_value, a = np.nan, np.nan, np.nan

        result[i,:] = np.nan, np.nan, np.nan, np.nan, np.nan

#Save result
lhs['PR_kappa_A[bmimpf6,R32]']=result[:,0]
lhs['PR_kappa_A[R32,bmimpf6]']=result[:,1]
lhs['PR_kappa_B[bmimpf6,R32]']=result[:,2]
lhs['PR_kappa_B[R32,bmimpf6]']=result[:,3]
lhs['SSR']=result[:,4]

lhs.to_csv('Data/Fits/LHS_Fits/PR_linTdep_LHS.csv',sep=',')