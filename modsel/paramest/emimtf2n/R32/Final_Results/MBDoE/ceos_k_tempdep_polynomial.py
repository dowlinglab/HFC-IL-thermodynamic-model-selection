...
class Cubic(EoSBase):
    ...
    def common(b, pobj):
        ...
        def rule_delta(m, p, i):
            ...
            kappa_D = getattr(m.params, cname+"_kappa_D")
            return (2*sqrt(a[i])/am[p] *
                    sum(m.mole_frac_phase_comp[p, j]*sqrt(a[j]) *
                        (1-(kappa_A[i, j] +
                            ...
                            kappa_D[i, j] * (m.temperature/m.params.temperature_ref)**3))
                ...
                return (2*sqrt(a[p1, p2, i])/am[p1, p2, p3] *
                        sum(m.mole_frac_phase_comp[p3, j]*sqrt(a[p1, p2, j]) *
                            (1-(kappa_A[i, j] +
                                ...
                                kappa_D[i, j] * (m._teq[p1,p2]/m.params.temperature_ref)**3))
    ...
    def build_parameters(b):
        ...
        if hasattr(param_block, cname+"_kappa_D"):
            # Common components already constructed by previous phase
            return

        kappa_D_data = param_block.config.parameter_data[cname+"_kappa_D"]
        param_block.add_component(
            cname+'_kappa_D',
            Var(param_block.component_list,
                param_block.component_list,
                within=Reals,
                initialize=kappa_D_data,
                doc=cname+'D binary interaction parameters',
                units=None))
    ...
    def log_fug_phase_comp_Tbub(blk, p, j, pp):
        ...
        kappa_D = getattr(blk.params, cname+"_kappa_D")
        am = sum(sum(x[xidx, i]*x[xidx, j]*sqrt(a(i)*a(j))*
                    (1-(kappa_A[i, j] +
                        ...
                        kappa_D[i, j] * (blk.temperature_bubble[pp]/blk.params.temperature_ref)**3))
    ...
    def log_fug_phase_comp_Tdew(blk, p, j, pp):
        ...
        am = sum(sum(x[xidx, i]*x[xidx, j]*sqrt(a(i)*a(j))*
                ...
                kappa_D[i, j] * (blk.temperature_dew[pp]/blk.params.temperature_ref)**3))
                for j in blk.component_list)
                for i in blk.component_list)
    ...
    def log_fug_phase_comp_Pbub(blk, p, j, pp):
        ...
        am = sum(sum(x[xidx, i]*x[xidx, j] *
                     ...
                     kappa_D[i, j] * (blk.temperature/blk.params.temperature_ref)**3))
                     for j in blk.component_list)
                 for i in blk.component_list)
...
def rule_am_default(m, cname, a, p, pp=()):
    ...
    k_D = getattr(m.params, cname+"_kappa_D")
    ...
    return sum(sum(
        ...
        sqrt(a[pp, i]*a[pp, j])*
            (1-(k_A[i, j] +
                ...
                k_D[i, j] * (m.temperature/m.params.temperature_ref)**3))
        for j in m.components_in_phase(p))
        for i in m.components_in_phase(p))
